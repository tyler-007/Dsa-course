...
int main()
{
    struct BTree* tree = createBTree(3); // Create a B-Tree with minimum degree 3
    ...
    // Search for a key
    char searchKey = 'o';
    struct BTreeNode* result = search(tree->root, searchKey);
    if (result != NULL) {
        printf("\n'%c' is present in the tree.", searchKey);
    } else {
        printf("\n'%c' is not present in the tree.", searchKey);
    }
